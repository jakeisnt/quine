...
class Path {
  ...
  join(nextPart: Path | string) {
    const nextPartStr = nextPart.toString();
    // If the next part is already a full absolute path (contains the current path),
    // just return it to avoid path duplication
    if (nextPartStr.startsWith(this.pathString) && this.pathString.length > 1) {
      return Path.create(nextPartStr);
    }
    return Path.create(this.pathString + nextPartStr);
  }
